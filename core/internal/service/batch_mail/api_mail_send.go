...
func sendApiMailWithSender(ctx context.Context, apiTemplate *entity.ApiTemplates, subject string, content string, log ApiMailLog, sender *mail_service.EmailSender) error {
        ...
        // Pool-first routing for transactional API mail
        if relay_pool.IsAutoRouteEnabled(ctx) && relay_pool.IsPoolAvailable(ctx) {
                _, poolErr := relay_pool.SendViaPool(ctx, poolMsg)
                if poolErr == nil {
                        return nil
                }
                if errors.Is(poolErr, relay_pool.ErrPoolExhausted) {
                        return relay_pool.ErrPoolExhausted
                }
                // transient error — fall through to direct SMTP
        }

        if err := sender.Send(message, []string{log.Recipient}); err != nil {
                // last-resort: try pool as fallback
                if !relay_pool.IsAutoRouteEnabled(ctx) && relay_pool.IsPoolAvailable(ctx) {
                        _, poolErr := relay_pool.SendViaPool(ctx, poolMsg)
                        if poolErr == nil {
                                return nil
